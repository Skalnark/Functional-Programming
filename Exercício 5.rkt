...
(define (map f lst)
  (define (tail-map f lst acc)
    (cond
       ...
       [(tail-map f (rest lst) (cons (f (first lst)) acc))]))
  (tail-map f lst '()))
...
(define (adiciona-prefixo pre lista-assuntos)
  (map
   (lambda (x)
     (if (equal? x "") (string-append(string-append "[" (string-append pre "]") x))
         (string-append(string-append "[" (string-append pre "] ") x))))
   lista-assuntos))
...
(define (fold-right f val lst)
  (if (empty? lst) val
    (fold-right f (f (first lst) val) (rest lst))))
...
(define (format x y)
  (cond
    [(equal? x "") y]
    [(string-append x sep y)]))

  (cond
    [(empty? lista-str) ""]
    [(equal? "" (first lista-str)) (cons (first lista-str)
                                         (fold-left format sep (rest lista-str)))]
    
    [(fold-left format "" lista-str)]))
